...
async def global_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------------------------------------------------
    # التحقق: المستخدم راكب + لا يوجد حالة معلقة + النص ليس زر أمر
    current_user_data = USER_CACHE.get(str(user_id), {})
    user_role = current_user_data.get('role', 'rider') # نفترض أنه راكب افتراضياً

    # نستثني المشرفين والسائقين من تفعيل الذكاء الاصطناعي في الخاص (اختياري)
    if user_role == 'rider' and not state and update.message.chat.type == "private":
        
        # رسالة انتظار عشان الراكب يعرف ان فيه تحليل جاري
        wait_msg = await update.message.reply_text("🤖 جاري قراءة طلبك.. لحظة بس..")
        
        # 1. استدعاء دالة التحليل (تأكد أنك أضفتها في الدوال المساعدة)
        ...
        # 2. إذا اكتشف الذكاء الاصطناعي أنه طلب مشوار حقيقي
        if ai_result.get('is_order') and ai_result.get('district'):
            district = ai_result['district']
            
            # البحث في قاعدة البيانات عن كباتن في هذا الحي
            ...
            if drivers:
                await update.message.reply_text(
                    f"✅ **تم فهم طلبك!**\n\n"
                    f"📍 الحي: {district}\n"
                    f"🚕 جاري إبلاغ {len(drivers)} كابتن قريبين منك الآن..."
                )
                
                # إرسال الطلب للسائقين
                await send_order_to_drivers(drivers, text, user)
                return # نخرج من الدالة عشان ما تروح الرسالة للدعم الفني
            else:
                await update.message.reply_text(
                    f"⚠️ **عذراً..**\n"
                    f"فهمت انك تبي مشوار من ( {district} ) بس ما عندي كباتن مسجلين بهذا الحي حالياً."
                )
                return # نخرج أيضاً
        
        # 3. إذا كان الكلام سوالف عادية أو استفسار --> يكمل الكود وينزل للدعم الفني بالأسفل
